	...
	Properties{
		...
		SubShader{
			...
			Pass {
				...
				StructuredBuffer<float> rotBuffer;
				...
				v2f vert(appdata_t i, uint instanceID : SV_InstanceID, uint vertexID : SV_VertexID) {
					...
					float2 t = posBuffer[instanceID];
					float r = rotBuffer[instanceID];
					float s = scaleBuffer[instanceID];

					float4 transform = float4(t.x, t.y, r, s);
